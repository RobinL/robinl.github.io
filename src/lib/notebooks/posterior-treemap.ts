...
export function renderPosteriorTreemap(prior: number, m: number, u: number, scenario: string, width: number, stage: 1 | 2 | 3): HTMLElement {
  ...
  if (stage === 1) nodes = [root, ...(root.children ?? [])];
  ...
  const svg = d3.select(container).append('svg').attr('height', height).attr('width', stage === 3 ? actualWidth : width);
  if (stage === 3 && nodes.length === 2) {
    const firstWidth = nodes[0].x1 - nodes[0].x0;
    const firstHeight = nodes[0].y1 - nodes[0].y0;
    const secondWidth = nodes[1].x1 - nodes[1].x0;
    const secondHeight = nodes[1].y1 - nodes[1].y0;
    const highestY = Math.min(nodes[0].y0, nodes[1].y0);
    nodes[0].y0 = highestY;
    nodes[0].y1 = highestY + firstHeight;
    nodes[1].y0 = highestY;
    nodes[1].y1 = highestY + secondHeight;
    nodes[1].x0 = nodes[0].x1 + 20;
    nodes[1].x1 = nodes[1].x0 + secondWidth;
  }
...
export function posteriorFormula(prior: number, m: number, u: number): HTMLElement {
  ...
  const priorFormatted = prior.toFixed(2);
  const mFormatted = m.toFixed(2);
  const uFormatted = u.toFixed(2);
  katex.render(String.raw`\text{posterior match probability}=\frac{${priorFormatted}\times ${mFormatted}}{${priorFormatted}\times ${mFormatted}+(1-${priorFormatted})\times ${uFormatted}}=${posterior(prior, m, u).toFixed(2)}`, element, {throwOnError: false});
