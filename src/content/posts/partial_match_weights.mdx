...
codeUrl: "https://github.com/RobinL/robinl.github.io/blob/dev/src/notebooks/partial-match-weights.notebook.html"
probLinkageCategory: "tutorial"
tutorialNumber: 2
pageWidth: "prose"
---
...
import { Vega, VegaLite } from '../../components/probabilistic/Vega.jsx';
...
For this reason, we call each scenario a '<DefinitionToolTip term="comparison_level" text="comparison level" />'.

It is up to the modeller to define these comparison levels and the resultant partial match weights they wish to estimate.

### Example:  Defining <DefinitionToolTip term="comparison_level" text="comparison levels" /> for a first name column

For first name, we could define the following three <DefinitionToolTip term="comparison_level" text="comparison levels" />:

1. Exact match: First names are identical
2. Fuzzy match:  First names are similar according to the Jaro Winkler measure, see [here](https://en.wikipedia.org/wiki/Jaro%E2%80%93Winkler_distance#Jaro%E2%80%93Winkler_similarity).
3. All other cases
...
<VegaLite client:visible spec={partial_match_weights_first_name} />
...
We can refer to this group of <DefinitionToolTip term="comparison_level" text="comparison levels" /> as a <DefinitionToolTip term="comparison" />.

### Remaining columns

Similarly, we can define <DefinitionToolTip term="comparison" text="comparisons" /> for the remaining columns and estimate the partial match weights of each of their <DefinitionToolTip term="comparison_level" text="comparison levels" />.

The number of <DefinitionToolTip term="comparison" text="comparisons" /> modelled and their definition can vary depending on the type of information.
...
<VegaLite client:visible spec={partial_match_weights} />
...
Let's take a look at an example record <DefinitionToolTip term="comparison" />, and see how the partial match weights are used.
...
<NotebookCellProvider client:visible notebook="partial-match-weights">
