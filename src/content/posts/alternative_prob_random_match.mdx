...
codeUrl: "https://github.com/RobinL/robinl.github.io/blob/dev/src/mdx/alternative_prob_random_match.mdx"
...
To compute the match probability for a specific pairwise comparison, we then update our beliefs based on the information in the records using <a href="/posterior_treemap/">Bayesian conditioning</a>.
...
Recall from <a href="/maths_of_fellegi_sunter/">our article on the maths of the Fellegi Sunter model</a> that:

$\text{posterior odds of match} = \frac{\lambda }{1- \lambda}K_1 K_2 \dots K_n$

where $$K_i$$ is the Bayes Factor for the activated comparison level of the $$i$$th comparison.

The relevant Bayes Factors are 20 and 50, so we have:

$$ \text{posterior odds of match} = \left( \frac{\frac{50,000}{4,999,950,000}}{1 - \frac{50,000}{4,999,950,000}} \right) \times 50 \times 20$$
