...
The previous article showed how `m` and `u` probabilities <a href="/m_and_u_values/#mathematical-annex">can be converted</a> into Bayes Factors, which in turn could be combined with the  <DefinitionToolTip term="prior" /> probability to compute an updated prediction (the 'posterior' probability).
...
In the  <a href="/intro_to_probabilistic_linkage/">first article</a> in this tutorial we claimed that the Fellegi-Sunter model makes its predictions by summing of the partial match weights.

This follows from equation 4.1.  Specifically, by applying $\log_2$ to both sides, you get:
...
In the <a href="/m_and_u_values/#mathematical-annex/">previous article</a> we saw that by applying Bayes Theorem to a specific scenario we could write:
...
By repeated application of Bayes Theorem (see annex <a href="/visualising_fellegi_sunter/">here</a>), we can generalise this formula to:
