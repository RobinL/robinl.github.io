...
codeUrl: "https://github.com/RobinL/robinl.github.io/blob/dev/src/notebooks/fault-tolerant-trie.notebook.html"
probLinkageCategory: "other_articles"
---
...
<aside class="post-alert" role="note" aria-label="Methodology update">
  <strong>Methodology update:</strong> Further testing on real-world data showed that trie-based address matching was less effective than we had hoped.

  We couldn't find a set of fault-tolerance rules that worked better than probabilistic approaches:
  - Permissive fault-tolerance rules matched more addresses but were less accurate than Splink-based fuzzy matching,
  - Stricter rules achieved high precision only for a small number of records beyond exact string matching. The number was so small it didn't justify the additional complexity of the trie-based approach.

  I have left this blog post up because it's possible an extension to the base methodology could make a breakthrough in terms of accuracy.
...
<NotebookCellProvider client:visible notebook="fault-tolerant-trie">

<NotebookCell cellName="example_trie_for_blog" />
...
<NotebookCell cellName="test_addresses" />


We can look up a messy address in the trie as follows.  Again, this is ✨interactive✨.

<NotebookCell cellName="messy_address_to_lookup" />
<NotebookCell cellName="trie_with_route" />
