...
More generally, we can see from the formula that strong positive match weights only possible with low `u` probabilities, implying high <DefinitionToolTip term="cardinality" text="cardinality" />.
...
- Amongst non matching records, gender will not match around half the time.  So $u = 0.5$.
