...
codeUrl: "https://github.com/RobinL/robinl.github.io/blob/dev/src/notebooks/intro-prob-linkage.notebook.html"
...
This is part one of an interactive introduction to the theory of probabilistic record linkage and deduplication.

In this first article I provide a high-level introduction to the <DefinitionToolTip term="fellegi-sunter" text="Fellegi-Sunter"/> framework and an interactive example of a linkage model.
...
More broadly,  it is often impossible to classify pairs of records as matches or non-matches beyond any doubt. Instead, the aim of probabilisitic record linkage is to quantify likelihood that a pair of records refer to the same entity by considering evidence in favour and against a match and weighting it appropriately.

The most common set up for a probabilistic record linkage model is called the Fellegi-Sunter model.
...
For example, a match on postcode gives us more evidence in favour of a match than a match on gender, since the latter is much more likely to occur by chance.
...
<NotebookCellProvider client:visible notebook="intro-prob-linkage">
    ...
    Finally, an important note of caution regarding probabilities: the Fellegi Sunter model tends to do a good job of producing an ordinal score that tells us how much the two records match: higher scores mean better matches.  However, the numeric probabilities produced by the model are often poor estimates of the true probability of a match due to some of the statistical assumptions made by the model.  As such, I recommend working with the match weights produced by the model, rather than converting them to probabilities.
