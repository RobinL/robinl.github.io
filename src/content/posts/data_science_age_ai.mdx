...
The rise of LLMs has been the biggest shakeup to the profession that I have experienced.  The work has changed radically.  I used to spend most of my time programming, but in the last year I’ve barely written a single line of code[^coding_abilities].

[^coding_abilities]:Despite this, I believe my coding abilities remain as valuable as ever.  I still review a lot of code and design the architecture, but rarely write the individual lines. More on this from Simon Willison [here](https://news.ycombinator.com/item?id=45512098) and [here](https://simonwillison.net/2026/Jan/8/llm-predictions-for-2026/#6-years-typing-code-by-hand-will-go-the-way-of-punch-cards)

## How our job has changed

How has AI changed our role? **Our job is increasingly to guide AI to good solutions, not to write code ourselves.**[^matt]  The challenge is to translate problems into feedback loops that reliably steer agents in the right direction.

[^matt]: This is a paraphrase of ideas discussed in Matt Rocklin's [AI Zealotry](https://matthewrocklin.com/ai-zealotry/#feedback) post.

These feedback loops are crucial to the effective use of AI:  they are our main defence against slop and hallucinations, making reliable results more a matter of skill rather than luck.

This new skill emphasises judgement and critical thinking. At a higher level, though, our job remains the same. Whether we write the code ourselves or guide an agent, the crucial element is understanding what we’re aiming for and knowing what good looks like.

## Feedback loops

Luckily, data scientists are well placed to design these loops because the underlying skills are familiar:  building solutions has always been an iterative process of running experiments and rigorously evaluating performance.

The feedback system itself rarely has to be explicitly developed[^verification]: AI agents are already very good at establishing a verification loop without further instruction.  The skill is translating the problem into tasks of appropriate size based on the best feedback loop we can think of.  **The stronger the feedback loop, the larger the task we can safely delegate.**

[^verification]: For instance, agents will run your unit test suite without being told to, they'll design a website then view it as an image to see if it looks good, and so on.

Let’s take a look at what this looks like in practice, via a series of real-world examples that aim to illustrate the most effective ways I have found to use agents, ranging from simple performance improvements through to semi-autonomous research projects.

In these examples, pay attention to the interplay of two elements of the feedback loop:

- How the agent is able to generate new task-specific evidence (not in its training data), usually by running code.
...
In this example, the new evidence being generated by the agent is
1) Profiling results such as function call counts and execution times
2) Benchmarking results that measure speed following changes to the code

The verification is that Splink’s test suite passes.
...
A key problem in this kind of game is balance.  You don’t want one gun or one strategy to dominate.  But balancing the game is hard, with 7 types of gun, each with 16 upgrades.
...
This is another example of my favourite type of agentic work: a simple prompt results in a huge amount of work.  The agent had to build a computer player capable of self-play and then run lots of CPU intensive scripts and analyse the result. But the final change to the game code was tiny: just changing the damage constants on each gun[^maze].


[^maze]: Of course, this wasn’t perfect - the agent didn’t find more advanced strategies that break the game, such as building a ‘maze’ of walls.
...
An ideal function would have two important properties, as shown in the following image:
...
I directed the agent to start by collecting as much data as possible on that things that sound the same: pairs of words that should share the same code and pairs that should not.  For instance, one source was a machine-readable dictionary with pronunciation guides. The agent could use it to create pairs that ‘sound the same’ and pairs that ‘sound different’, with a training and test set. Similarly, I got it to find lists of place names in other countries with different transliterations.
...
Guiding it over the course of about 20 prompts, the agent was able to complete this research project for me, replacing perhaps two or more weeks of human work.  It discovered a function that substantially outperformed Soundex, and marginally outperformed DoubleMetaphone.
...
Of course, we need to be careful the AI is not overfitting here.  Human judgement was also important to this example: by reviewing real-world examples of true and false positives, I could see the feature makes intuitive sense as an important signal, increasing my confidence that it can genuinely increase accuracy.
...
I’ve been working on a [cursive handwriting library](https://www.robinlinacre.com/letterpaths/) for my kids that can help power educational apps.  This is a very visual piece of work where everything depends on the shapes of the letters and the joins between them.   LLMs, so far at least, are pretty weak at this.
...
- Cloud agents will become the norm, making it easier to run multiple agents in separate disposable environments[^work].
- This results in a stronger sandbox, reducing the need for approvals, and hence allowing longer-running autonomous work
...
- Agents will become better at managing memory and context, remembering much more about the full history of a project: past experiments, successes, failures, and project conventions and preferences.
- Agents will become better and understanding intent and recognising when they are off track or need input from the user

[^work]:I really like ChatGPT Work for this (the [Cloud version](https://simonwillison.net/2026/Aug/30/understanding-chatgpt-work/)).  I prefer the interface to an IDE for many tasks and think the application is a good indication of the direction of travel for tooling.

The result is that individual data scientists will increasingly be managing a team of agents.

## Summing up

I find this all in equal parts worrying and exciting.  But stepping back, programming was always the means rather than the end.    The natural extension to "how does my agent know it's doing good work" is "how do I know I'm doing good work".

The value of data scientists remains in our creativity and judgement: understanding business problems, translating them into technical implementations, and working iteratively with our customers towards better solutions.

As implementation gets cheaper, we will have to accept that the business will legitimately expect us to deliver more, faster.  If the new technical skill is building tight agentic feedback loops, the bigger-picture skill for data scientists will be to also accelerate the feedback loop with our customers, so we can deliver more valuable products more quickly.
