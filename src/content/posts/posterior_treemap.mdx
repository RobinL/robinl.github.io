...
codeUrl: "https://github.com/RobinL/robinl.github.io/blob/dev/src/notebooks/posterior-treemap.notebook.html"
...
<NotebookCellProvider client:visible notebook="updating-a-prior-simplified">
    ...
    $\text{match probability} = \frac{\colorbox{#ffff22}{\text{yellow area}}}{\colorbox{#ffff22}{\text{yellow area}} + \colorbox{#35cbff}{\text{blue area}}}$

    We can see that the $\colorbox{#ffff22}{\text{yellow area}=\(\lambda m\)}$ and the $\colorbox{#35cbff}{\text{blue area}=\((1-\lambda)u\)}$.

    So:

    $\text{match probability} = \frac{\colorbox{#ffff22}{\(\lambda m\)}}{\colorbox{#ffff22}{\(\lambda m\)} + \colorbox{#35cbff}{\((1-\lambda)u\)}}$
