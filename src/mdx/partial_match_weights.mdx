...
description: "Partial match weights in the Fellegi-Sunter model.  Part of an interactive introduction to probabilistic record linkage (data deduplication) theory"
...
In the previous article we saw that the Fellegi-Sunter model makes its predictions with a simple calculation: the sum of the partial match weights.
...
For first name, we could define the following three scenarios (in Splink, these are known as `ComparisonLevels`):
...
These scenarios are a series of mutually exclusive categories that work like a sequence of `if` statements:
