...
In this sense, a Bayes Factor is are similar to the concept of odds. Odds of 5 mean that an event happens five out of six times, or in some sense it is five times more likely for the event to happen than not happen.
...
One important takeaway from this example is that strong negative match weights require very low `m` probabilities, which in turn implies high data quality.
