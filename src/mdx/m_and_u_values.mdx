...
In this sense, a Bayes Factor is are similar to the concept of odds. Odds of 5 to 1 mean that an event happens five out of six times, or in some sense it is five times more likely for the event to happen than not happen.
...
For example, suppose we believe 1 in 120 records comparisons are a match. But now we observe the new information that month of birth matches, with a Bayes Factor of 12.

So, the posterior odds are:
$\text{posterior odds} =  \frac{1}{120}  \times 12 = \frac{1}{10}$
...
Supposing we observe the scenario that first name matches
