...
- The `m` probability is:  $$\text{Pr}(\text{scenario}|\text{records match})$$

- The `u` probability is: $$\text{Pr}(\text{scenario}|\text{records do not match})$$

What matters is the _relative_ size of the m and u probabilities.  This is calculated as a ratio known as the Bayes Factor, denoted by $K$.

$\text{Bayes Factor} = K = \frac{m}{u} = \frac{\text{Pr}(\text{scenario}|\text{records match})}{\text{Pr}(\text{scenario}|\text{records do not match})} $

Bayes Factors are the easiest way to interpret the parameters of the Fellegi Sunter model because of they act as a relative multiplier to increase or decrease the overall prediction of a match. For example:
...
In this sense, a Bayes Factor is are similar to the concept of odds. Odds of 5 mean that an even happens five out of size times, or in some sense it is five times more likely for the event to happen than not happen.
...
We observe this scenario around 11.9 times more often amongst matching records than non-matching records.

Hence, given this observation, the records are 11.9 times more likely to be a match.

More generally, strong positive match weights are dependent on low `u` probabilities.

### Example 2: Evidence against a match

Suppose we observe that gender does not match.

- Amongst matching records, it will be rare to observe a non-match on date of birth.  If there are occasional data entry errors, we may have $m = 0.02$
- Amongst non matching records, gender will not around half the time.  So $u = 0.5$.
...
One important takeaway from this example is that strong negative match weights require very low `m` probabilitiess, implying high data quality.
...
Why do we need the additional concept of partial match weights useful when we already have Bayes Factors?  The are two main reasons:
...
A larger, standalone version is here:


## Bayes Factors and the prior

I've mentioned that Bayes Factors have an intuitive interpretation as relative multipliers.

But what does it mean for a match to be $n$ times more or less likely?  More likely than what?

It's only meaningful to say that something is more or less likely relative to a starting point - known as the 'prior' (our 'prior belief').

In the context of record linkage, the prior is our existing belief that the two records match before we saw the new information (e.g. that first names match).
...
Now that we have a good understanding, we're in a positon to present the full mathematical specification of the Fellegi Sunter model.

## Mathematical annex

We can use Bayes Theorem to write:

$\text{posterior} = \frac{\operatorname{Pr}(\text{first name matches}|\text{prior})(\text{prior})}{\operatorname{Pr}{(\text{first name matches})}}$

$\text{posterior} = \frac{m \times \text{prior}}{m \times \text{prior} + u \times (1 - \text{prior})}$
