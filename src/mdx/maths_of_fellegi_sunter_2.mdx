...
We can also think more generally than just matches and non-matches: partial match weights can be estimated for additional scenarios like 'not an exact match, but similar'.

It is up to the modeller to define the partial match weights they wish to compute.  For example, for first name, we could define the following three categories (in Splink, these are known as `ComparisonLevels`):
