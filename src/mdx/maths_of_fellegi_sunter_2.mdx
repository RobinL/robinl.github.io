...
In the chart, we can see that the partial match weight associated with an exact match on postcode is stronger than the partial match weight associated with gender.

This should be intuitive: a match on gender doesn't tell us much about whether two records are a match, whereas a match on postcode is much more informative.
...
Let's take a look at how the partial match weights in the chart above are applied.

Consider the following <mark>interactive</mark> table of data. <mark>The below calculations will update as you edit it</mark>.
...
<WithObservableProvider notebook={notebook}>
    ...
    This is a reproduction of the above partial match weights chart with the activated partial match weights highlighted in colour.

    <ObservableCell cellName="partial_match_weights_highligh" />

    This corresponds to the following waterfall chart:
    <ObservableCell cellName="waterfall_highlight" />

    And finally we can represent the calculation of overall match weight as a formula:
...
We now have a good qualitative understanding of the meaning partial match weights. But how are they calculated and how should we interpret their magnitude?
