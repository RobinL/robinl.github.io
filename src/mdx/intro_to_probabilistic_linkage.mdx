...
<WithObservableProvider notebook={notebook}>
    ...
    We can represent the sum of the <DefinitionToolTip term="partial_match_weight" />s graphically using a waterfall chart, which is read from left to right. We start with the prior, and take each column into account in turn.  The size of the bar corresponds to the {<DefinitionToolTip term="partial_match_weight" />}.
