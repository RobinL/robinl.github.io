...
<Subtitle>
  Introducing Splink: A fast, accurate and scalable record linkage library that
...
- Has a simple but highly customisable interface, so users can tackle the majority of record linking and deduplication problems

- Training data is not required because models can be trained using an unsupervised approach.

- Supports the full workflow of data linkage, from exploratory analysts, through to model prediction, diagnostics and quality assurance.
...
In a nutshell, Splink takes messy data that may looks like this:
...
And produces pairwise predictions of which records match:
