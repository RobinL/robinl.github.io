...
[^2]: With 192 core processors such as [this](https://www.tomshardware.com/pc-components/cpus/amd-launches-epyc-turin-9005-series-our-benchmarks-of-fifth-gen-zen-5-chips-with-up-to-192-cores-500w-tdp) available in the cloud and only costing around $15,000, the complexity of clusters can largely be avoided.  There is actually now a distributed version of DuckDB, see [here](https://mehdio.substack.com/p/duckdb-goes-distributed-deepseeks)

This post sets out some of my favourite features of DuckDB that set it apart from other SQL-based tools.    In a nutshell, it's simple to install, ergonomic, fast, and more fully featured than other SQL-based tools.
...
- 'In-process' means its similar to SQLite in that it runs within your application. You don't need to start a separate service such as Postgres to run it.
...
The performance difference of of analytics-optimised engines ([OLAP](https://en.wikipedia.org/wiki/Online_analytical_processing)) vs. transactions-optimised engines ([OLTP](https://en.wikipedia.org/wiki/Online_transaction_processing)) should not be underestimated. A query running in DuckDB can be 100 or even 1,000 times faster than exactly the same query running in (say) SQlite or Postgres.
...
In DuckDB high performance custom UDFs can be written in C++.  Whilst writing these functions is certainly not trivial, DuckDB [community extensions](https://duckdb.org/2024/07/05/community-extensions.html) offers a very low-friction way of distributing the code. Community extensions can be installed almost instantly with a single command such as `INSTALL h3 FROM community` to install hierarchical hexagonal indexing for geospatial data.
