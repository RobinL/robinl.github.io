...
The previous article showed how `m` and `u` probabilities <Link to="/m_and_u_values/#mathematical-annex">can be converted</Link> into Bayes Factors, which in turn could be combined with the prior  <DefinitionToolTip term="prior" /> probability to compute an updated prediction (the 'posterior' probability).
