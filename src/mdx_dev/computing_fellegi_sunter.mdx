...
The aim of the model is to compute a prediction of which records match. This prediction is a probability which quantifies the likelihood the two records represent the same entity.
...
## Computing the prediction step by step
    ...
    ### Input table 1
    <ObservableCell cellName="input_table_l" />

    ### Input table 2:
    <ObservableCell cellName="input_table_r" />




    In the first step of the calculation we set values against each other, as pairwise record comparisons:

    <ObservableCell cellName="comparison_pairs_no_gamma" />

    At first glance, it seems challenging to turn this table of text into a numeric prediction.  But this can be achieved using the concepts of <DefinitionToolTip term="scenario" />s and <DefinitionToolTip term="partial_match_weight" />s <Link to="/partial_match_weights/">introduced earlier in this tutorial</Link>.
    ...
    <ObservableCell cellName="lookup_table" />
    ...
    We then proceed by computing which scenario is <DefinitionToolTip term="activate" />d for each pairwise record comparison, and representing this using the comaprison vector value.  We use the gamma symbol (γ) to refer to the comparison vector value.

    <ObservableCell cellName="comparison_pairs_with_gamma" />
    ...
    We now look up the partial match weights from the comparison vector.  I have also added the <DefinitionToolTip term="prior" />, expressed as a partial match weight, and finally the sum:
    ...
    <ObservableCell cellName="cv_pmw_sum" />
    ...
    <ObservableCell cellName="raw_values_with_final_match_prob" />
