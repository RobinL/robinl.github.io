...
This article describes the three types of information that are most important, and how they are all leveraged by the Fellegi-Sunter model as used by [Splink](https://github.com/moj-analytical-services/splink).
...
The most obvious way to predict whether these records represent the same entity is to mesaure whether the columns contain the same or similar information.

Similarity can be measured quantitatively using fuzzy matching functions like [Levenshtein](https://en.wikipedia.org/wiki/Levenshtein_distance) or [Jaro-Winker](https://en.wikipedia.org/wiki/Jaro%E2%80%93Winkler_distance) for text, or numeric differences such as absolute or percentage difference.
...
However using this approach alone has major drawback: **the weights are arbitrary** in two ways:
...
This concept can be extended to encompass similar records that are not an exact match.  Specifically, the weight is derived from an estimate of how common it is to observe fuzzy matches across the dataset.  In probabilistic linkage, this information is captured in the `u` probabilities which are discussed in more detail in the  <Link to="/m_and_u_values/">article on m and u probabilities</Link>.
...
Much of the power of probabilistic models comes from properly combining all three sources of information in a way which is not possible in other models.

Specifically, probabilistic models explicitly estimate  <Link to="/partial_match_weights/">partial match weights</Link> which use a data-driven approach to capture the relative importance, and therefore properly combine, of all the different types of information.

Conversely, fuzzy matching techniques often use arbitary weights, and cannot fully incorporate relevant information from the source datasets. Term frequency approaches lack the ability to use information about data quality, or a mechnicanism to appropriately weight fuzzy matches.